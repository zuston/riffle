...
use crate::system_libc::send_file;
use crate::urpc::command::ReadSegment;
use crate::urpc_uring::command::{
    ...
    GetMemoryDataResponseCommand, GetMemoryDataResponseV2Command, RpcResponseCommand,
    ...
    pub async fn write(
        ...
        match frame {
            Frame::GetLocalDataResponse(resp) => {
                ...
                let header_data = write_buf.split();
                let (res, _) = stream.write_all(header_data).await;
                res?;

                // write all data
                match data {
                    DataBytes::Direct(val) => {
                        let (res, _) = stream.write_all(val.clone()).await;
                        res?;
                    }
                    DataBytes::Composed(val) => {
                        let (res, _) = stream.write_all(val.freeze()).await;
                        res?;
                    }
                    DataBytes::RawIO(raw) => {
                        todo!()
            ...
            Frame::GetLocalDataIndexV2Response(resp) => {
                ...
                let (res, _) = stream.write_all(data.clone()).await;
                res?;
            ...
            Frame::GetMemoryDataResponse(resp) => {
                ...
                for composed_byte in data_bytes_wrapper.always_composed().iter() {
                    let (res, _) = stream.write_all(composed_byte.clone()).await;
                    res?;
                }
                Ok(())
            }
